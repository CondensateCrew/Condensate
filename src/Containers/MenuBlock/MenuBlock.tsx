...
import {IChangeBrainstormAction, IDeleteBrainstormAction } from 'interfaces';
...
const MenuBlock: React.FC<Props> = ({setIsClicked, id}) => {
  ...
  const toggleCard = (): IChangeBrainstormAction => dispatch(toggleBrainstorm(id));
  const deleteCard = (): IDeleteBrainstormAction => dispatch(deleteBrainstorm(id));
  return (
